...
щоб колір, заради якого все робиться, лишався видним. Ставиться в токен (урок 07 — `ui/kit.css`, з уроку 08 — `--filter-desaturate` у `tokens/tokens.css`), у змінній
...
`components/photo.css` (модифікатори `.photo--*`; урок 07 — `ui/kit.css`) і в `wireframes/_archive/_panel.css` · повторна перевірка
