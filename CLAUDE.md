...
- **Мова на вайрфреймах:** [`wireframes/_panel.css`](./wireframes/_panel.css) поверх
  `_wireframe.css` — guided-matching (4 стани) і solution-detail (3 стани), два кінці
  головного стрибка; спільні компоненти звірені відбитками (бейдж/кнопки/поверхні —
  байт-у-байт). Решта 21 сторінка свідомо сіра. Розмітка й текст не змінені: стани
  розпізнаються через `:has()` по смузі станів.
