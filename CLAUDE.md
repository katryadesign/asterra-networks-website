...
- **Кіт (плаский):** [`ui/kit.css`](./ui/kit.css) — змінні в `:root` + класи компонентів, голих
  значень у класах немає (числа лише в умовах `@media`); стан і варіант — явний клас, не
  `:has()`. Оболонка як розмітка — [`ui/shell.html`](./ui/shell.html) (skip-link, двошарова
  шапка, підвал, підключення шрифтів). Вітрина — [`ui/kit.html`](./ui/kit.html). Іконки Solar
  linear локально — `ui/icons/`. Розколу токенів на рівні немає — це урок 08.
...
- **Екрани:** усі 28 сторінок підключають тільки шрифти, `../ui/kit.css` і
  [`wireframes/_chrome.css`](./wireframes/_chrome.css) (службовий хром: дерево, смуга станів,
...
Коли користувач каже **«залишаємо»** про візуальну правку:

1. **Онови кіт** — значення в змінних `:root` файлу [`ui/kit.css`](./ui/kit.css), розмітку
   компонента у вітрині [`ui/kit.html`](./ui/kit.html).
2. **Підпиши причину в [`DESIGN.md`](./DESIGN.md)** — у розділі компонента, з датою.
3. **Перенеси зміну розмітки на всі екрани**, де компонент стоїть. Зміна значення в `kit.css`
   розноситься сама; зміна розмітки — ні, і правка на одному екрані = розсинхрон.

Екрани не мають власних стилів. Бракує компонента чи варіанта — спершу в кіт, вітрину й
[`ui/inventory.md`](./ui/inventory.md), потім на екран. Змінюєш текст — спершу
«Поточний канон» у `microcopy.md`.
