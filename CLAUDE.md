...
- **Мова на вайрфреймах:** [`wireframes/_archive/_panel.css`](./wireframes/_archive/_panel.css) поверх
...
- **Геометричний прохід по рендеру** (2026-08-27, після скріна користувача «сиро»):
  7 агентів за протоколом [`wireframes/_archive/_parts/_GEOMETRY.md`](./wireframes/_archive/_parts/_GEOMETRY.md) —
  вимір прихованим iframe на 1440/1024/375, сім класів «сирості», правки лише у
  власних фрагментах зі звітом «було → стало». Наскрізний корінь трьох незалежних
  вимірів (фото +8px за трек: стеля clamp 32px проти поля 40px) виправлено в
  `_panel.css`; гігієнічна четвірка і стик approval — рішення оркестратора. Фінальний
  зведений прогін: 28 + головна × 1440/375 — нуль провалів (контраст · кегль ≥12.8 ·
  переповнення · стики героя · трек фото · дерево). Журнал:
...
## 19. Збірка UI (урок 07, stage complete 2026-09-17)

Екрани `wireframes/` зібрано з кіта. Мова задокументована з коду, не з голови.

- **Мова:** [`DESIGN.md`](./DESIGN.md) — згенеровано `/impeccable document` з пофарбованих
  екранів (guided matching, solution detail) і далі синхронізовано з кітом на кожному кроці;
  таблиці «Правки після критики» (крок 6) і «Правки перевірки» (крок 7) — причини змін.
  `concept/concept.md` іде за макетами: Р1 панелі з рамкою · Р2 на екранах рішення носій —
  фото · Р3 морська зелень як матеріал у знімку, в UI хроматики не додано.
- **Інвентар:** [`ui/inventory.md`](./ui/inventory.md) — компоненти всього продукту з
  вайрфреймів, таблиця «старе → кіт», реєстр варіантів.
- **Кіт:** на уроці 07 — плаский `ui/kit.css` (змінні в `:root` + класи); на уроці 08 розколотий
  на [`tokens/`](./tokens/) і [`components/`](./components/) і видалений — див. §20. Стан і
  варіант — явний клас, не `:has()`. Оболонка як розмітка — [`ui/shell.html`](./ui/shell.html)
  (skip-link, двошарова шапка, підвал, порядок підключення). Вітрина —
  [`ui/kit.html`](./ui/kit.html). Іконки Solar linear локально — `ui/icons/`.
- **Візуали:** [`visuals/`](./visuals/) — 9 власних знімків (Gemini · Nano Banana) за
  промптом [`visuals/README.md`](./visuals/README.md): обладнання як об'єкт, один колорит,
  синій відлив нейтралізовано. Оригінали генерації — `visuals/_source/` (у git не йдуть).
- **Екрани:** усі 28 сторінок на уроці 07 підключали тільки шрифти, `../ui/kit.css` і
  [`wireframes/_chrome.css`](./wireframes/_chrome.css) (з уроку 08 — токени й компоненти, §20) (службовий хром: дерево, смуга станів,
  анотація). Перевід звірено автоматично: 11 445 порівнянь елементів на 1440/1024/375 — 0
  розбіжностей з вихідним рендером. Старі `_wireframe.css`, `_panel.css`, `_parts/` —
  [`wireframes/_archive/`](./wireframes/_archive/) (не підключені, джерело значень).
- **Текст:** [`microcopy.md`](./microcopy.md), розділ **«Поточний канон»** — знятий з екранів
  після кроку 7, перемагає старіші таблиці «було/стало». Вигадані марки партнерів і правило
  дат — [`voice.md`](./voice.md), «Рішення уроку 07».
- **Процес:** [`ui/_process/`](./ui/_process/) — протоколи паралельних субагентів (кроки 5–6),
  зведена критика (5 груп, `/impeccable critique`) і таблиця дефектів кроку 7
  (`/impeccable audit` 15/20).
- **Свідомо відкрите:** знахідки [Н] кроку 5 (шапка без межі, підкреслені посилання підвалу,
  дата новини системним моно) · активний пункт навігації не позначений · валідація поля й
  error без live-region · P3 з `ui/_process/defects.md` і R5–R18, E2–E22 з
  `ui/_process/critique.md` — не в пріоритеті уроку 07.

### Правило «залишаємо»

Коли користувач каже **«залишаємо»** про візуальну правку (оновлено уроком 08, крок 5 — значення
тепер живуть у [`tokens/tokens.css`](./tokens/tokens.css), класи — у [`components/`](./components/)):

1. **Правка значення — у токен відповідного рівня** `tokens/tokens.css`, не в компонент:
   - **колір** — у semantic-роль (`--bg-*`, `--text*`, `--rule*`, `--action`, `--on-action`,
     `--event`) або `--solo-*`; якщо значення нове — спершу primitive (`--gray-NN`), потім роль
     посилається на нього. Перевір пару і в блоці `[data-theme="light"]`, контраст — коментарем;
   - **геометрія** (відступ, розмір, кегль, радіус) — у primitive (`--space-N`, `--size-N`,
     `--font-size-N`…); семантичного рівня для геометрії немає.
   Компонент ніколи не отримує hex, px чи назву шрифту, і не читає колір-primitive напряму.
2. **Зміна розмітки — у вітрину [`ui/kit.html`](./ui/kit.html) і на всі екрани**, де компонент
   стоїть. Зміна значення розноситься сама; зміна розмітки — ні, і правка на одному екрані =
   розсинхрон. Новий клас — у файл свого компонента `components/<компонент>.css`.
3. **Підпиши причину в [`DESIGN.md`](./DESIGN.md)** — у розділі компонента, з датою.

Екрани не мають власних стилів: кожна сторінка підключає лише шрифти, `../tokens/tokens.css`,
`../components/index.css` і службовий `_chrome.css` (+ `_theme.js`). Бракує компонента чи
варіанта — спершу в `components/`, вітрину й [`ui/inventory.md`](./ui/inventory.md), потім на
екран. Змінюєш текст — спершу «Поточний канон» у `microcopy.md`.

## 20. Токени (урок 08, stage complete 2026-09-17)

Плаский кіт розколото на два рівні токенів і файли компонентів. **Вигляд не змінився**: перед
видаленням `ui/kit.css` усі 28 сторінок звірено з ним автоматично — 84 звірки на 1440/1024/375,
22 629 елементів, 0 розбіжностей. Змінено потім і свідомо лише правками кроку 6 (нижче).

- **Аудит:** [`tokens/tokens-audit.md`](./tokens/tokens-audit.md) — 328 змінних кіта: значення,
  де вживається, роль у кожному місці; дрейф (А1–А10), одна змінна на кілька ролей (Б1–Б10),
  значення повз змінні (В1–В4), кандидати в ролі.
- **Токени:** [`tokens/tokens.css`](./tokens/tokens.css) — два рівні.
  - **Primitive** — сирі значення без ролей, назва = значення: `--gray-12…97`, `--red-34/68`,
    `--space-N`, `--size-N`, `--font-size-N`, `--leading-*`, `--tracking-*`, `--radius-2`,
    `--stroke-1/2`, `--duration-*`, `--solar-*` (іконки). Кожен рядок — коментар, з яких
    змінних кіта прийшов. Дрейф зведено лише там, де звірка підтвердила, що геометрія не
    рухається; решта — `ui/_process/critique.md` §6.2.
  - **Semantic** — **лише ролі кольору**, назви з мови DESIGN.md: `--bg-ground` · `--bg-panel` ·
    `--bg-band` · `--bg-panel-on-band` · `--bg-well` · `--text` · `--text-quiet` ·
    `--text-label` · `--rule` · `--rule-rank` · `--rule-gap` · `--action` · `--on-action` ·
    `--event`. Кожна — коментар, з яких вживань аудиту виросла. «Довіри» й «небезпеки» кольором
    немає: довіра — ранг і чуже ім'я, стан — іконка й слово.
  - **`--solo-*`** — колір одного місця, ще не роль (межа поля, ховер дії, межа помилки,
    кістка скелета, затемнення заяви, шеврон). Кожен записано в `critique.md` §6.1.
  - **Геометрія семантичного рівня не має** — компонент читає primitive напряму.
- **Компоненти:** [`components/`](./components/) — 40 файлів, один на компонент, і
  [`components/index.css`](./components/index.css) (порядок `@import` = порядок каскаду).
  Колір — лише semantic або `--solo-*`; hex, px і назв шрифтів у класах немає; числа лишились
  тільки структурні й в умовах `@media` (CSS не приймає там `var()`). 40 `@import` — прийнятно
  для прототипу, на білді бандлиться (I1).
- **Стрес-тест теми:** блок `[data-theme="light"]` перевизначає лише semantic і `--solo-*`,
  тільки наявними primitive; `components/` для теми не змінювався. Перемикач — службовий
  [`wireframes/_theme.js`](./wireframes/_theme.js) у смузі станів кожного екрана. Контраст
  тексту AA в обох темах на всіх 28 сторінках (скан кожного елемента). Знайдена діра — колір
  шеврона в SVG — закрита варіантом primitive. **Рішення користувача: тема лишається тестом,
  не фічею продукту**; продукт dark-first (`color-scheme: dark`).
- **Службовий хром** (`_chrome.css`, `_theme.js`) бере кольори з primitive напряму: інструмент
  не перефарбовується темою.
- **Перевірка кроку 6:** [`ui/_process/defects-08.md`](./ui/_process/defects-08.md) —
  `/impeccable audit` 17/20. Виправлено всі P1 і P2:
  - **C1** межа рангу `--rule-rank` gray-42 → **gray-55** (2.40 → 4.18:1, WCAG 1.4.11; A15 уроку 07);
  - **N1** `--solo-tier-fill` злито в `--rule-rank`; `--rule-gap` лишився gray-42;
  - **N4** кегль 0.94rem і 0.9375rem → **0.875rem** (мета, кнопка, головна навігація, панелі
    величин документа, квитанції й сторінки) — закрило й дрейф А1; скелет кнопки рахує висоту
    з метрик кнопки;
  - **I2** `color-scheme: dark` / `light`; **D1** документи без посилань на видалений кіт.
- **Свідомо відкрите:** P3 з `defects-08.md` (N2, N3, M1, H1–H3, C2, I3–I5, D2) · `--solo-*`
  чекають рішення «роль чи стан» (урок 09) · дрейф D2–D7, D9 (`critique.md` §6.2) · фокус і
  стани як ролі — урок 09 · C3 (подія на світлому) не актуальне, поки тема — тест.
