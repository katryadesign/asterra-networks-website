...
(`ui/kit.css` + вітрина `ui/kit.html`). Нічого не вигадано: кожен рядок має адресу в розмітці.
...
не використовує жоден екран `wireframes/`, у `ui/kit.css` не має права з'явитися.

## 13. Назви в кіті — старе (вайрфрейми) → нове (ui/kit.css)
...
Кожна сторінка підключає тільки `../ui/kit.css` + `_chrome.css`; `<style>` і `style=""` —
