...
- **Rank Line** (`line-strong`, #4d4d4d): межа рангу — первинна картка, перша клітина тріо
...
- **Body-sm** (400, 0.94rem): `dd`, `.meta`, значення spec, головна навігація, текст кнопки (600).
...
- **Primary:** `ink` фон, `bg` текст, межа `ink`, 0.94rem/600, падінг 11×18px, gap 8px.
...
### [TBD] slot
- **Style:** Value, `ink-2`, пунктирна межа 1px `line-strong`, 2px, падінг 2×8px.
...
- **Шапка двошарова, без фону й межі [Н]:** бренд 1rem/600 стоїть **окремим рядком над**
  ...
  утилітний ряд 0.8rem `ink-2`, головний 0.94rem/400 `ink`.
...
за правилом «залишаємо»: значення в `ui/kit.css`, розмітка у вітрині `ui/kit.html` і на всіх
екранах, причина — тут.
