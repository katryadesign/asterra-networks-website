...
| Pillar (one of 5) | brief §5 **[FACT — brief]** |
...
| Field | Basis |
|---|---|
| Type: type approval · ISO cert · norm compliance · stat · support commitment · outcome case | Navarino, Marlink, Speedcast patterns (`research.md` §3) **[AUDIT]** |
...
⚠️ **Priority note:** if the primary flips to P2, R5 drops from the MVP core
...
| **Insights / News article** | Maps to no job; "every competitor has it" is not an argument (`jtbd.md` §8.3) | A documented job it serves |
...
1. **P1 vs P2 primary** (`personas.md` §10.1) — flips relative weight of O6 (rises) and
   O8 (drops); O1, O2, O7 are stable under either answer (`jtbd.md` §8.2).
...
│  CTA here (brief §6)
│     (MAIN — the start of orientation)  [PRIMARY + SECONDARY]  → O3 + O4
│     · the five-pillar (O1) list lives here as a SECTION, not a
...
·  Deliberately NOT a screen. R2 is table stakes (jtbd.md §8.2) and
   is served by O5 Proof Items distributed as attributes on solution,
   industry and crew screens under the E2 single-source constraint.
   A standalone "trust page" would detach proof from the object it
   proves.
...
└─ Approval materials screen — ungated library of forwardable,
   dated, self-consistent artifacts [?]
      (R3 — primary; E2 as its quality constraint)
      [SECONDARY (P2 prepares → P4 receives); P1 = 2]  → O6
      · [?] because R3's primary carrier is the forwardable solution
        detail screen; a separate library is backed by the matrix row
        (jtbd.md §8.1 R3 function) but its contents are [?]
        (personas.md §7.4). Rises in priority if primary flips to P2
        (jtbd.md §8.2).
...
- **Solution overview screen — removed, merged into the Entry screen as a section.**
  ...
  component rule, a five-pillar list without its own job is a section. Promotion
...
|---|---|---|---|---|---|
| 1 | **"Find what fits"** (guided entry) | Guided matching → solution details | **MAIN** (`jtbd.md` §2) — the product's declared job; "without it the site has no point" (§8.2.1) | P1 ⭐ + P2 (both = 3 in the matrix) | The core job must be reachable from anywhere — the buyer arrives mid-task from any screen. ⚠️ its *lead position* vs item 2 carries the §5 priority caveat |
...
- *A "Solutions" catalog item* — the five-pillar list is a **section of the entry
...
| Approval materials [?] | **DEEP** | R3; secondary personas (P2→P4, P1=2); needed once per purchase, late in the cycle; hangs off solution detail. Elevating it now would invert persona priority without evidence. **Onward path: → Inquiry** (contextual) — recorded at verification; without it the R3 flow ended in the model's only hidden dead end |
